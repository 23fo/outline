import { escape } from "es-toolkit/compat";
import type { Context } from "koa";
import { withBasePath } from "@shared/utils/subpath";
import env from "@server/env";

/**
 * Resolves a browser redirect URL against the configured application subpath.
 *
 * @param url the redirect URL.
 * @param basePath the configured application base path.
 * @returns the normalized redirect URL.
 */
export function resolveClientRedirectUrl(
  url: string,
  basePath: string
): string {
  return url.startsWith("/") ? withBasePath(url, basePath) : url;
}

/**
 * Performs a redirect on the browser so that the user's auth cookies are
 * included in the request. Assigned to the Koa context as `redirectOnClient`.
 *
 * @param url the URL to redirect to.
 * @param method the HTTP method to use for the redirect. Use POST when
 * preventing links in emails from being clicked by bots. Otherwise, use GET.
 */
export function redirectOnClient(
  this: Context,
  url: string,
  method: "GET" | "POST" = "GET"
) {
  this.type = "text/html";
  const redirectUrl = resolveClientRedirectUrl(url, env.basePath);

  if (method === "POST") {
    // For POST method, create a form that auto-submits
    const urlObj = new URL(redirectUrl, this.request.origin);
    const formAction = `${urlObj.origin}${urlObj.pathname}`;
    const searchParams = urlObj.searchParams;

    let formFields = "";
    searchParams.forEach((value, key) => {
      formFields += `<input type="hidden" name="${escape(
        key
      )}" value="${escape(value)}" />`;
    });

    // Bots are shown a manual submit button instead of auto-submitting, so that
    // link scanners in email clients don't follow the redirect.
    const isBot = this.userAgent.isBot;

    if (isBot) {
      formFields += `
          <p>If you are not redirected automatically, please click the button below.</p>
          <input type="submit" value="Continue" />
        `;
    }

    const script = isBot
      ? ""
      : `<script nonce="${this.state.cspNonce}">
    document.getElementById('redirect-form').submit();
  </script>`;

    this.body = `
<html lang="en">
<head>
  <title>Redirecting…</title>
</head>
<body>
  <form id="redirect-form" method="POST" action="${formAction}">
    ${formFields}
  </form>
  ${script}
</body>
</html>`;
  } else {
    // Default GET method using meta refresh
    this.body = `
<html lang="en">
<head>
<meta http-equiv="refresh" content="0;URL='${escape(redirectUrl)}'" />
</head>
</html>`;
  }
}
